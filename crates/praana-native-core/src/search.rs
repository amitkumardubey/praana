...
fn walk_files(root: &Path) -> NativeResult<Vec<PathBuf>> {
    if !root.exists() {
    ...
    if !root.is_dir() {
        return Err(NativeError::new(
            ...
            format!("not a directory or file: {}", root.display()),
        ));
    }
...
fn compile_globset(patterns: &[String], label: &str) -> NativeResult<Option<GlobSet>> {
    ...
    builder.build().map(Some).map_err(|e| {
        NativeError::new(
            NativeErrorCode::InvalidArgument,
            format!("invalid {label} glob set: {e}"),
        )
    })
}
...
fn matches_globs(rel: &str, include: Option<&GlobSet>, exclude: Option<&GlobSet>) -> bool {
    ...
    match include {
        None => true,
        Some(inc) => {
            inc.is_match(rel) || inc.is_match(Path::new(rel).file_name().unwrap_or_default())
        }
    }
}
...
fn compile_pattern(pattern: &str, case_insensitive: bool) -> NativeResult<(Regex, Option<String>)> {
        ...
        Err(e) => {
            ...
            let re = RegexBuilder::new(&literal)
                ...
                .map_err(|e2| {
                    NativeError::new(
                        NativeErrorCode::InvalidArgument,
                        format!("invalid regex: {e2}"),
                    )
                })?;
            Ok((
                re,
                Some(format!("invalid regex ({e}); searched as literal")),
            ))
        }
    }
}

pub fn grep(opts: GrepOptions) -> NativeResult<GrepOutput> {
    let root = opts.path.clone();
    let files = walk_files(&root)?;

    let include = compile_globset(opts.globs.as_deref().unwrap_or(&[]), "include")?;
    let exclude = compile_globset(opts.glob_exclude.as_deref().unwrap_or(&[]), "exclude")?;

    let case_insensitive = opts.case_insensitive.unwrap_or(false);
    let (re, regex_fallback) = compile_pattern(&opts.pattern, case_insensitive)?;
...
pub fn find_files(opts: FindFilesOptions) -> NativeResult<FindFilesOutput> {
    let root = opts.path.clone();
    let files = walk_files(&root)?;
    let walk_root = if root.is_file() {
        root.parent().unwrap_or(&root).to_path_buf()
...
mod tests {
    ...
    fn fixture() -> tempfile::TempDir {
        ...
        .unwrap();
        fs::write(
            dir.path().join("src/hello.txt"),
            "before-line\nTARGET line\nafter-line\n",
        )
        .unwrap();
        fs::write(
            dir.path().join("node_modules/lib.ts"),
