...
fn kind_for_def_node(node_kind: &str, lang: LangId) -> &'static str {
    match (lang, node_kind) {
        ...
        (
            _,
            "lexical_declaration" | "variable_declaration" | "variable_declarator" | "assignment",
        ) => "variable",
        _ => "other",
    }
}
    ...
    match lang {
        ...
        LangId::Python => {
            ...
            for child in node.children(&mut cursor) {
                if matches!(
                    child.kind(),
                    "dotted_as_names" | "dotted_as_name" | "aliased_import"
                ) {
                    collect_identifiers(child, source, &mut names);
                }
            }
...
pub fn extract_references(
    ...
    while let Some(m) = matches.next() {
        for cap in m.captures {
            ...
            let name = cap.node.utf8_text(parsed.source.as_bytes()).unwrap_or("");
            if name != symbol {
                continue;
            }
...
"#
        ...
        assert!(
            imports
                .iter()
                .any(|i| i.source.contains("Bar") || i.source.contains("foo")),
            "{imports:?}"
        );
    }
}
