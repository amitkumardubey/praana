...
function freezeLive(state: TranscriptState): TranscriptState {
  ...
  let entry = state.live;
  // Thinking entries only get durationMs stamped in thinking_close, which fires
  // only when tool calls follow. For tool-free turns the entry is frozen here
  // via assistant_delta — stamp duration now so "Thought · Xs" renders correctly.
  if (entry.role === "thinking" && entry.thinkingStartedAt !== undefined && entry.durationMs === undefined) {
    entry = { ...entry, durationMs: Date.now() - entry.thinkingStartedAt };
  }
  return {
    ...state,
