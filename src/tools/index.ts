...
const SHARED_TOOL_DESCRIPTIONS = [
  ...
  "search_code(pattern, path?, glob?, glob_exclude?, case_insensitive?, context?, max_results?, file_type?, timeout?) — Structured native code search (file:line:column matches with context and stats)",
  "find_files(pattern, mode?, path?, max_results?, timeout?) — Fuzzy file path search via the native addon (typo-resistant fuzzy or pure glob mode)",
