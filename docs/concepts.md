...
Tree-sitter `code_*` tools (#11 Phase 1) provide syntax diagnostics and name-based symbol queries via `@praana/natives`. The opt-in LSP tier (#11 Phases 2–3) adds diagnostics, formatting, hover, completions, semantic definition/references, and list+apply code actions against external servers configured under `[lsp]` (disabled by default; `format_on_edit` is also opt-in). Use `code_*` for fast in-project name queries; `lsp_definition` / `lsp_references` when you need types, stdlib, or `node_modules`.
