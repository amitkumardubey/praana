...
Tree-sitter `code_*` tools (#11 Phase 1) provide syntax diagnostics and name-based symbol queries via `@praana/natives`. The opt-in LSP tier (#11 Phase 2) adds `lsp_diagnostics` / `lsp_format` against external servers configured under `[lsp]` (disabled by default; `format_on_edit` is also opt-in).
