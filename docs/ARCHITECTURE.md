...
- Embedder (`src/memory/embeddings.ts` + `src/memory/embedder-factory.ts` + `src/memory/transformers-embedder.ts`): supports `auto`, `transformers`, `transformers-nomic`, `ollama`, `llama-cpp`, and `hash`. `auto` uses Transformers.js when installed and falls back to hash with a warning. Models cache in `~/.praana/models/`.
